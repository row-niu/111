...
def csgd_prune_and_save(engine:Engine, layer_idx_to_clusters, save_file, succeeding_strategy, new_deps):
    ...
    for layer_idx, namedvalue in enumerate(kernel_namedvalues):
        ...
        print('Layer {} ({}): original shape={}, pruned shape={}, new_deps={}'.format(
            layer_idx, k_name, k_value.shape, kernel_value_pruned.shape, new_deps[layer_idx]))
        ...
        if succeeding_map is None or layer_idx not in succeeding_map:
            continue
        ...
        for follow_idx in follows:
            if isinstance(follow_idx, str) and follow_idx == 'linear1':
                # 处理 linear1 层（全连接层）
                follow_kernel_name = 'linear1.weight'
                follow_kernel_value = engine.get_param_value_by_name(follow_kernel_name)
                ...
                # 调整 linear1.weight 的输入维度
                fc_idx_to_delete = []
                num_filters = k_value.shape[0]  # conv13 的输出通道数
                ...
                if len(fc_idx_to_delete) > 0:
                    follow_kernel_value = delete_or_keep(follow_kernel_value, np.concatenate(fc_idx_to_delete, axis=0),
                                                         axis=1)
                ...
                if follow_kernel_value.ndim == 2:  # The following is a FC layer
                    ...
                    if len(fc_idx_to_delete) > 0:
                        follow_kernel_value = delete_or_keep(follow_kernel_value,
                                                             np.concatenate(fc_idx_to_delete, axis=0), axis=1)
                    result[follow_kernel_name] = follow_kernel_value
                    print('shape of pruned following kernel: ', follow_kernel_value.shape)
