...
def calculate_resB_bottleneck_flops(fd, resnet_n):
    ...
    for i in range(num_blocks[3]):
        ...
        # result.appendmmel(get_con_flops(d[4][i][1], d[4][i][2], 7, kernel_size=1))
...
def calculate_rc164_flops(deps):
    return calculate_rc_flops(deps, 27)

def calculate_vgg_flops(deps):
    result = []
    # Stage 1: 32x32
    result.append(get_con_flops(3, deps[0], 32, 32, kernel_size=3))
    result.append(get_con_flops(deps[0], deps[1], 32, 32, kernel_size=3))
    # Maxpool: 16x16
    result.append(get_con_flops(deps[1], deps[2], 16, 16, kernel_size=3))
    result.append(get_con_flops(deps[2], deps[3], 16, 16, kernel_size=3))
    # Maxpool: 8x8
    result.append(get_con_flops(deps[3], deps[4], 8, 8, kernel_size=3))
    result.append(get_con_flops(deps[4], deps[5], 8, 8, kernel_size=3))
    result.append(get_con_flops(deps[5], deps[6], 8, 8, kernel_size=3))
    # Maxpool: 4x4
    result.append(get_con_flops(deps[6], deps[7], 4, 4, kernel_size=3))
    result.append(get_con_flops(deps[7], deps[8], 4, 4, kernel_size=3))
    result.append(get_con_flops(deps[8], deps[9], 4, 4, kernel_size=3))
    # Maxpool: 2x2
    result.append(get_con_flops(deps[9], deps[10], 2, 2, kernel_size=3))
    result.append(get_con_flops(deps[10], deps[11], 2, 2, kernel_size=3))
    result.append(get_con_flops(deps[11], deps[12], 2, 2, kernel_size=3))
    # FC layers
    result.append(deps[12] * 512)  # linear1: deps[12]*1*1 -> 512
    result.append(512 * 10)       # linear2: 512 -> 10
    return np.sum(np.array(result, dtype=np.float32))