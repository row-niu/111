...
if __name__ == '__main__':
    ...
    local_rank = int(os.environ.get('LOCAL_RANK', 0))  # 默认值为 0
    ...
    if network_type == 'sres50':
        ...
        base_lrs = LRSchedule(base_lr=5e-2, max_epochs=600, lr_epoch_boundaries=[200, 350, 500], lr_decay_factor=0.1,

                              linear_final_lr=None, cosine_minimum=None)

        train_base_config = get_baseconfig_by_epoch(network_type=network_type,
                                                    dataset_name=get_dataset_name_by_model_name(network_type),
                                                    dataset_subset='train',

                                                    global_batch_size=batch_size, num_node=1,

                                                    weight_decay=weight_decay_strength, optimizer_type='sgd',
                                                    momentum=0.9,

                                                    max_epochs=base_lrs.max_epochs, base_lr=base_lrs.base_lr,
                                                    lr_epoch_boundaries=base_lrs.lr_epoch_boundaries,

                                                    lr_decay_factor=base_lrs.lr_decay_factor,
                                                    cosine_minimum=base_lrs.cosine_minimum,
                                                    ...
                                                    linear_final_lr=base_lrs.linear_final_lr,

                                                    weight_decay_bias=0, deps=deps)

        warmup_epochs = 0

        lrs = LRSchedule(base_lr=3e-2, max_epochs=50, lr_epoch_boundaries=[200, 400], lr_decay_factor=0.1,
    ...
    elif network_type == 'src110':
        weight_decay_strength = 2e-4
        batch_size = 128
        deps = rc_origin_deps_flattened(18)
        succeeding_strategy = rc_succeeding_strategy(18)
        pacesetter_dict = rc_pacesetter_dict(18)
        base_log_dir = 'src110_train'
        init_hdf5 = None
        flops_func = calculate_rc110_flops
        base_lrs = LRSchedule(base_lr=5e-2, max_epochs=600, lr_epoch_boundaries=[200, 400, 550], lr_decay_factor=0.1,
                              linear_final_lr=None, cosine_minimum=None)
        train_base_config = get_baseconfig_by_epoch(network_type=network_type,
                                                    dataset_name=get_dataset_name_by_model_name(network_type),
                                                    dataset_subset='train',
        ...
        warmup_epochs = 5
        lrs = LRSchedule(base_lr=6e-2, max_epochs=600, lr_epoch_boundaries=[200, 400,550], lr_decay_factor=0.1,linear_final_lr = None, cosine_minimum = None)
        itr_deps = [[d * 13 // 16 for d in deps],
                    [d * 11 // 16 for d in deps],
                    [d * 5 // 8 for d in deps]]
        centri_strength = 3e-3

    elif network_type == 'src164':
        weight_decay_strength = 2e-4
        batch_size = 128
        deps = rc_origin_deps_flattened(27)
        succeeding_strategy = rc_succeeding_strategy(27)
        pacesetter_dict = rc_pacesetter_dict(27)
        base_log_dir = 'src164_train'
        init_hdf5 = None
        flops_func = calculate_rc164_flops
        base_lrs = LRSchedule(base_lr=5e-2, max_epochs=400, lr_epoch_boundaries=[200, 350, 550], lr_decay_factor=0.1,
                              linear_final_lr=None, cosine_minimum=None)
        train_base_config = get_baseconfig_by_epoch(network_type=network_type,
                                                    dataset_name=get_dataset_name_by_model_name(network_type),
                                                    dataset_subset='train',
                                                    ...
                                                    weight_decay_bias=0, deps=deps)
        warmup_epochs = 5
        lrs = LRSchedule(base_lr=6e-2, max_epochs=600, lr_epoch_boundaries=[200, 400, 550], lr_decay_factor=0.1,
                         linear_final_lr=None, cosine_minimum=None)
        itr_deps = [[d * 13 // 16 for d in deps],
                    [d * 11 // 16 for d in deps],
                    [d * 5 // 8 for d in deps]]
        ...
        succeeding_strategy = 'simple'  # VGG层结构简单，使用默认的后续策略
        pacesetter_dict = None  # VGG没有复杂的层依赖
        base_log_dir = 'vgg_train'
        init_hdf5 = None  # 从头训练
        flops_func = calculate_vgg_flops
        base_lrs = LRSchedule(base_lr=3e-2, max_epochs=300, lr_epoch_boundaries=[100,220], lr_decay_factor=0.1,
                              linear_final_lr=None, cosine_minimum=None)  # 学习率调整为 0.03，衰减因子为 0.1
        train_base_config = get_baseconfig_by_epoch(network_type=network_type, dataset_name='cifar10',
                                                    dataset_subset='train',
                                                    global_batch_size=batch_size, num_node=1,
                                                    weight_decay=weight_decay_strength, optimizer_type='sgd',
                                                    momentum=0.9,
                                                    ...
                                                    lr_decay_factor=base_lrs.lr_decay_factor,
                                                    cosine_minimum=base_lrs.cosine_minimum,
                                                    warmup_epochs=0, warmup_method='linear', warmup_factor=0,  # 移除预热
                                                    ckpt_iter_period=20000, tb_iter_period=100, output_dir=base_log_dir,
                                                    tb_dir=base_log_dir, save_weights=None, val_epoch_period=2,
                                                    linear_final_lr=base_lrs.linear_final_lr,
                                                    weight_decay_bias=0, deps=deps)

        warmup_epochs = 5  # 移除预热
        lrs = LRSchedule(base_lr=3e-2, max_epochs=600, lr_epoch_boundaries=[200,400], lr_decay_factor=0.1,
                         linear_final_lr=None, cosine_minimum=None)  # 学习率调整
        itr_deps = [[d * 14 // 16 for d in deps],
                    [d * 12 // 16 for d in deps],
                    [d * 10 // 16 for d in deps],
                    [d * 9 // 16 for d in deps],
                    ]

        centri_strength = 3e-3  # 与论文一致
    else:
    ...
    csgd_config = get_baseconfig_by_epoch(network_type=network_type,
                                      ...
                                      warmup_epochs=warmup_epochs, warmup_method='linear',
    ...
    #                begin_itr=int(start_arg.begin_itr))
    if local_rank == 0:
        print('=====================================')
        print('prune {}, the original deps is {}, flops is {}'.format(network_type, deps, flops_func(deps)))
        for itr, cur_deps in enumerate(itr_deps):
    ...
    csgd_iterative(local_rank=local_rank, init_hdf5=init_hdf5, base_train_config=train_base_config,
                   csgd_train_config=csgd_config, itr_deps=itr_deps, centri_strength=centri_strength,
                   pacesetter_dict=pacesetter_dict, succeeding_strategy=succeeding_strategy,
                   begin_itr=int(start_arg.begin_itr))